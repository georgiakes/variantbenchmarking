...
workflow VARIANTBENCHMARKING {
    ...
    fai = Channel.fromPath(params.fai, checkIfExists: true)
        .map { fai -> tuple([id: fai.getSimpleName()], fai) }
        .collect()

    //// check Truth Files ////

    if (params.truth_vcf || params.regions_bed) {
        truth_ch = params.truth_vcf
            ? Channel.fromPath(params.truth_vcf, checkIfExists: true).map { vcf -> tuple([id: params.truth_id, vartype: params.variant_type], vcf) }.collect()
            : Channel.empty()

        regions_bed_ch = params.regions_bed
            ? Channel.fromPath(params.regions_bed, checkIfExists: true).collect()
            : Channel.empty()
        targets_bed_ch = params.targets_bed
            ? Channel.fromPath(params.targets_bed, checkIfExists: true).collect()
            : Channel.empty()
    }
    else {
    ...
    falsepositive_bed = params.falsepositive_bed
        ? Channel.fromPath(params.falsepositive_bed, checkIfExists: true).map { bed -> tuple([id: "falsepositive"], bed) }.collect()
        : Channel.of([[id: "falsepositive"], []]).collect()
    ambiguous_beds = params.ambiguous_beds
        ? Channel.fromPath(params.ambiguous_beds, checkIfExists: true).map { bed -> tuple([id: "ambiguous"], bed) }.collect()
    ...
    sdf = params.sdf
        ? Channel.fromPath(params.sdf, checkIfExists: true).map { sdf -> tuple([id: sdf.getSimpleName()], sdf) }.collect()
        : Channel.empty()
    ...
    vcf_ch = out_vcf_ch.mix(
        SUBSAMPLE_VCF_TEST.out.vcf_ch,
    ...
    PREPARE_VCFS_TEST.out.vcf_ch
        .combine(PREPARE_VCFS_TRUTH.out.vcf_ch)
