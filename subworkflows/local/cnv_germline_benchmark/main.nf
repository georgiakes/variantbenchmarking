...
workflow CNV_GERMLINE_BENCHMARK {
    ...
    input_ch
        .map { meta, _vcf, _tbi, _truth_vcf, _truth_tbi, _regionsbed, _targets_bed ->
            [meta, _regionsbed]
        }
        .set { bed }
    ...
    WITTYER(
        TABIX_BGZIP_QUERY.out.output.join(TABIX_BGZIP_TRUTH.out.output, failOnMismatch: true, failOnDuplicate: true).join(bed, failOnMismatch: true, failOnDuplicate: true)
    )
